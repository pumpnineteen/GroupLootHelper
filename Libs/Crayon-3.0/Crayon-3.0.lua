...
Name: Crayon-3.0
Revision: $Rev: 1 $
Author(s): ckknight, pump
Description: A library to provide coloring tools.
]]

--Theondry added the purple.  yell at me if it's wrong, please

local MAJOR_VERSION = "Crayon-3.0"
local MINOR_VERSION = "1"

assert(LibStub, MAJOR_VERSION .. "requires LibStub")
local Crayon = LibStub:NewLibrary(MAJOR_VERSION, MINOR_VERSION)
if not Crayon then return end
...
function Crayon:ColorizeRGB(red, green, blue, text)
    local hexColor = Crayon:RGBtoHexColor(red, green, blue)
    return self:Colorize(hexColor, text)
end
...
function Crayon:RGBtoHexColor(red, green, blue)
...
function Crayon:GetThresholdHexColor(quality, ...)
	...
	return Crayon:RGBtoHexColor(r, g, b)
end
